...
test.describe('Search and Help Features', () => {
    ...
    test('should highlight search results in window body', async ({ page }) => {
        ...
        const window = page.locator('[data-testid="window-about_me.doc"]');
        ...
        // Search for a term that exists (e.g., "skills")
        await searchInput.fill('skills');

        // Check for highlighted text (using <mark> tag)
        const highlight = window.locator('mark');
        await expect(highlight).toBeVisible();
        await expect(highlight).toContainText(/skills/i);
    ...
    test('help menu should only contain "About" and open an about window', async ({ page }) => {
        ...
        // Verify a NEW window titled "About About_Me.doc" (or similar) opens
        const aboutWindow = page.locator('[data-testid^="window-about-about_me.doc"]');
